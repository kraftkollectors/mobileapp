...
const SplitRevTab = ({ data, total }) => {
  let level = (data.rate / total) * 100;

  return (
    <View style={styles.srTab}>
      <View style={styles.groupTab}>
        <Text style={styles.groupType}>{data?.value}</Text>
        <Octicons name="star-fill" size={12} color={COLORS.black400} />
      </View>

      <View style={styles.percentTrack}>
        <View
          style={[styles.percentLevel, { width: level > 0 ? `${level}%` : 0 }]}
        ></View>
      </View>

      <Text style={styles.curTotal}>{data.rate}</Text>
    </View>
  );
};
    ...
    <View style={styles.splitBlock}>
      {reviewRatings && reviewRatings.length > 0 ? (
        reviewRatings.map((item, index) => (
          <SplitRevTab key={index} data={item} total={totalReviews} />
        ))
      ) : (
        <></>
      )}
